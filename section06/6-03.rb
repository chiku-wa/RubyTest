...
def section_06_03_04
  ...
  # 「?:」で単なるグループとした(キャプチャではなくマッチした文字列全体を取得したい)場合も、同様に配列として取り出される
  ...
  # slice!を使うと、正規表現に一致した文字破壊的に取り除く
  ...
  # 【注意！】第二引数を「"」で囲むと、キャプチャが参照できない！
  puts birthday.gsub(/(\d+)年(\d+)月(\d+)日/, "\1-\2-\3") #=> 誕生日は--です。
  # →ダブルクォーテーションの中で「\X」と記すと、Xそのものを指すため
